...
<!--
 ...
 2018-06-22 SMR copied from geoportal 1.2.9 github master
  adopted to make sure a title and abstract gets inserted so getCap docs with no title can work
  necessary for OneGeology Geonetwork.
  * update logic for finding elements to use local-names, reduce dependency on having the correct
     namespaces
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	xmlns:ows20="http://www.opengis.net/ows/2.0" xmlns:gml="http://www.opengis.net/gml"
	xmlns:csw="http://www.opengis.net/cat/csw" xmlns:csw202="http://www.opengis.net/cat/csw/2.0.2"
	xmlns:csw300="http://www.opengis.net/cat/csw/3.0" xmlns:wcs="http://www.opengis.net/wcs"
	xmlns:wcs11="http://www.opengis.net/wcs/1.1" xmlns:wcs111="http://www.opengis.net/wcs/1.1.1"
	xmlns:wfs="http://www.opengis.net/wfs" xmlns:wms="http://www.opengis.net/wms"
	xmlns:wps100="http://www.opengis.net/wps/1.0.0" xmlns:sos10="http://www.opengis.net/sos/1.0"
	xmlns:sps="http://www.opengis.net/sps" xmlns:tml="http://www.opengis.net/tml"
	xmlns:sml="http://www.opengis.net/sensorML/1.0.1" xmlns:myorg="http://www.myorg.org/features"
	...
	<xsl:template name="main">
		<rdf:RDF xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
			...
			xsl:exclude-result-prefixes="ows ows11 wms wps100 swe myorg tml sml sps sos10 wfs wcs wcs11 wcs111 csw csw202 csw300 gml">
			<rdf:Description>
				...
				<dc:title>
					<xsl:choose>
						<xsl:when
							test="string-length(//*[translate(local-name(), $uppercase, $smallcase) = 'title'][1]) > 0">
							<xsl:value-of
								select="//*[translate(local-name(), $uppercase, $smallcase) = 'title'][1]"
							/>
						</xsl:when>
						<xsl:when
							test="string-length(//*[local-name() = 'Service'][1]/*[local-name() = 'name'][1]) > 0">
							<xsl:value-of
								select="//*[local-name() = 'Service'][1]/*[local-name() = 'name'][1]"
							/>
						</xsl:when>
						<xsl:when test="string-length($sourceUrl) > 0">
							<xsl:value-of
								select="translate(substring-after($sourceUrl, 'http://'), '/:?', '--:')"
							/>
						</xsl:when>
						<xsl:otherwise>
				...
				<dc:description>
					<xsl:choose>
						<xsl:when
							test="string-length(//*[translate(local-name(), $uppercase, $smallcase) = 'abstract'][1]) > 0">
							<xsl:value-of
								select="//*[translate(local-name(), $uppercase, $smallcase) = 'abstract'][1]"
							/>
						</xsl:when>
						<xsl:when
							test="string-length(//*[local-name() = 'Service'][1]/*[local-name() = 'description'][1]) > 0">
							<xsl:value-of
								select="//*[local-name() = 'Service'][1]/*[local-name() = 'description'][1]"
							/>
						</xsl:when>
						<xsl:otherwise>
				...
				<!--				<xsl:for-each select="//ows:OperationsMetadata//ows:Operation[@name='DescribeRecord']//ows:Parameter[@name='outputFormat']//ows:Value |	
				...
				<xsl:for-each
					select="
						...
						//wms:Capability//wms:Request//wms:GetCapabilities//wms:Format |
						//ows:Parameter[@name = 'AcceptFormats']//ows:Value">
<!--					<xsl:variable name="thepos" select="position()"/>
					<xsl:variable name="localname" select="local-name()"/>
					<xsl:variable name="thevalue" select="text()"/>-->
					<dc:format>
						<xsl:value-of select="text()"/>
					</dc:format>
				</xsl:for-each>
				...
				<xsl:for-each
					select="
						...
						/WMT_MS_Capabilities/Service//ContactPerson |
						//*[local-name() = 'responsibleParty']/*[local-name() = 'individualName'] |
						//*[local-name() = 'ServiceContact']/*[local-name() = 'IndividualName']">
				...
				<xsl:if test="string-length(normalize-space($sourceUrl)) > 0">
					<dc:identifier>
						<xsl:value-of select="$sourceUrl"/>
					</dc:identifier>
					<dct:references>
				...
				<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:for-each select="//*[local-name() = 'ServiceType']">
							<dc:type>
								<xsl:value-of select="text()"/>
							</dc:type>
						</xsl:for-each>
				...
				<xsl:for-each
					select="//*[translate(local-name(), $uppercase, $smallcase) = 'keyword']">
					<dc:subject>
						<xsl:value-of select="text()"/>
					</dc:subject>
				</xsl:for-each>
...
//wcs111:Capability/wcs111:Request/wcs111:GetCapabilities/wcs111:DCPType/wcs111:HTTP/wcs111:Post/wcs111:OnlineResource/@xlink:href">-->
				...
				<!-- simplify select using local names -->
				<xsl:for-each
					select="
						//Service/OnlineResource/@xlink:href |
						//*[local-name() = 'Operation'][@name = 'GetCapabilities']//@xlink:href |
						//*[local-name() = 'GetCapabilities']//*[local-name() = 'OnlineResource']/@xlink:href">
					<dct:references>
				...
				<xsl:choose>
					...
					<xsl:when
						test="//ows:LowerCorner | //ows11:LowerCorner | //ows20:LowerCorner | //gml:LowerCorner | //gml:pos[1] | //gml:coord[1] | //gml:lowerCorner | //gml:Envelope[@srsName = 'EPSG:4326']">
	...
	<xsl:template name="OWS_WGS84BoundingBox">
		<ows:WGS84BoundingBox
			xsl:exclude-result-prefixes="wms wps100 swe myorg tml sml sps sos10 wfs wcs wcs11 wcs111 csw csw202 csw300 gml">
			<xsl:choose>
				<xsl:when
					test="/wms:WMS_Capabilities/@version = '1.3.0' or /WMS_Capabilities/@version = '1.3.0' or /wms:WMT_MS_Capabilities/@version = '1.3.0' or /WMT_MS_Capabilities/@version = '1.3.0'">
					<ows:LowerCorner>
	...
	<xsl:template name="getLCMinx">
		<xsl:for-each
			...
			<xsl:sort select="number(normalize-space(substring-before(normalize-space(.), ' ')))"
	...
	<xsl:template name="WMS_SummaryBoundingBox">
		<xsl:param name="box"/>
		<ows:WGS84BoundingBox
			xsl:exclude-result-prefixes="wps100 swe myorg tml sml sps sos10 wfs wcs wcs11 wcs111 csw csw202 csw300 gml">
			<xsl:choose>
				<xsl:when
					test="/wms:WMS_Capabilities/@version = '1.3.0' or /WMS_Capabilities/@version = '1.3.0' or /wms:WMT_MS_Capabilities/@version = '1.3.0' or /WMT_MS_Capabilities/@version = '1.3.0'">
					<ows:LowerCorner>
